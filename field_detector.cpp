...
FieldDetector::FieldDetector(int width, int height, int *lutCb, int *lutCr)
...
void FieldDetector::proceed(
  ...
  if (borderPoints.size() >= 6) {
    ...
    if (pointsCnt >= 5) {
      ...
      // interpolate the field-border from two lines
      for (int x = 0; x < width; x++) {
...
const int *FieldDetector::getConvexFieldBorder() const {
