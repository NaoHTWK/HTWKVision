...
LineDetector::LineDetector(int width, int height, int *lutCb, int *lutCr):width(width),height(height){
	this->lutCb=lutCb;
	this->lutCr=lutCr;
...
//---------------------------------------
    ...
    for(const LineGroup &lg:linesList){
		...
		if(fabsf(angle1)>0.05&&fabsf(angle2)>0.05&&angle1*angle2>0){
			float nx=(leLeft1.nx+leRight1.nx-leLeft2.nx-leRight2.nx)*0.25;
			float ny=(leLeft1.ny+leRight1.ny-leLeft2.ny-leRight2.ny)*0.25;
			if(angle1<0){
				nx*=-1;
				ny*=-1;
			}
			float xm=(leLeft1.x+leRight1.x+leLeft2.x+leRight2.x)*0.25;
			float ym=(leLeft1.y+leRight1.y+leLeft2.y+leRight2.y)*0.25;
...
point_2d LineDetector::getIntersection(float px1, float py1, float vx1, float vy1, float px2, float py2, float vx2, float vy2) {
	...
	float sx=1./ab*(qb_*ax-pa_*bx);
	float sy=1./ab*(qb_*ay-pa_*by);
...
float LineDetector::getError2(LineSegment *le1, LineSegment *le2){
	...
	int nx=le1->x-le2->x;
	int ny=le1->y-le2->y;
	float r=1./(16+sqrtf(nx*nx+ny*ny));
