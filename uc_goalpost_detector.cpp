...
UpperCamGoalPostDetector::UpperCamGoalPostDetector(int8_t* lutCb, int8_t* lutCr, HtwkVisionConfig &config)
    : BaseDetector(lutCb, lutCr, config),
      imageWidth(config.ucGoalPostDetectorConfig.scaledImageWidth),
      imageHeight(config.ucGoalPostDetectorConfig.scaledImageHeight),
      patchesX(config.ucGoalPostDetectorConfig.patchesX),
      patchesY(config.ucGoalPostDetectorConfig.patchesY),
      patchWidth(imageWidth / patchesX),
      patchHeight(imageHeight / patchesY) {
    const int hypCount = patchesX * patchesY;

    const auto& gdConf = config.ucGoalPostDetectorConfig;
...
UpperCamGoalPostDetector::~UpperCamGoalPostDetector() {
    if (!config.ucGoalPostDetectorConfig.runDetector) {
        free(inputDetector);
    }
}

void UpperCamGoalPostDetector::proceed(CamPose& cam_pose, std::shared_ptr<ImagePreprocessor> imagePreprocessor) {
    Timer t("UpperCamGoalPostDetector", 50);
    EASY_FUNCTION();
    const float* img = imagePreprocessor->getScaledImage().data();

    EASY_BLOCK("UpperCamGoalPostDetector Prepare");
    ...
    EASY_BLOCK("UpperCamGoalPostDetector Hyp");
    detector.execute();
    EASY_END_BLOCK;
    ...
    for (int y = 0; y < patchesY; y++) {
        for (int x = 0; x < patchesX; x++) {
            ...
            if (res[0] < config.ucGoalPostDetectorConfig.probabilityThreshold)
                continue;
            ...
            auto radius = LocalizationUtils::getPixelRadius(h, cam_pose, 0.1f);
...
void UpperCamGoalPostDetector::drawPatch(uint8_t* yuvImage, int px, int py) {
